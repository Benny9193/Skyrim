...
function loadLocation(location) {
    ...
    difficultyEl.className = 'detail-value difficulty-badge ' + (location.difficulty || 'normal').toLowerCase().replace(' ', '-');
    
    document.getElementById('locationFastTravel').textContent = location.fastTravel ? 'Available ✓' : 'Not Available ✗';
...
function updateList(elementId, items) {
    ...
    if (!items || items.length === 0) {
        listEl.innerHTML = '<li style="color: var(--color-text-secondary); font-style: italic;">None</li>';
        return;
    }
